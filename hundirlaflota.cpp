...
void jugar()
{
	...
	case 2:
	{
		...
		strcpy_s(usuario2.nombre, LTEXTOS, "Máquina");
...
char leer_orientacion(char nombre[])
{
	...
	do
	{
		...
		// -> Optamos por leer cadenas para que la entrada sea más robusta <-
...
void barcos_maquina(char tablero[TAM][TAM], BARCO barcos[])
{
	...
	for (i = 0; i < NBARCOS; i++)
	{
		...
		// bucle para generar de manera aleatoria la posición de cada barco
		do
		{
			columna = rand() % 9;
			fila = rand() % 9;
		...
		for (j = 0; j < barcos[i].longitud; j++)
		{
			if (barcos[i].orientacion == 'H')         // Diferenciamos si la orientación fue Horizontal o Vertical para introducir un vector fila o un vector columna
				tablero[fila][columna + j] = barcos[i].simbolo;
			else
				tablero[fila + j][columna] = barcos[i].simbolo;
		}
...
POSICION posicion_de_disparo_usuario(int f, int c)
{
	...
	fila = f + 1; // En la primera fila están las coordendas
	columna = c + 3; // En la primera columna están las coordenadas, luego un espacio en blanco y luego el [
...
void fichero1(MEMORIA memoria1, MEMORIA memoria2,char tablero1[][TAM],char tablero2[][TAM])
{
	...
	strcpy_s(memoria2.nombre, LTEXTOS, "La Máquina");
